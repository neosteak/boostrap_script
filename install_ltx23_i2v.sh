...
WORKFLOW_DEST="${WORKFLOW_DIR}/ltx23_i2v_triple_stage.json"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
mkdir -p "${WORKFLOW_DIR}"
if [[ -f "${SCRIPT_DIR}/ltx23_triple_stage_workflow.json" ]]; then
  cp "${SCRIPT_DIR}/ltx23_triple_stage_workflow.json" "${WORKFLOW_DEST}"
  echo "[ltx23] workflow installed: ${WORKFLOW_DEST}"
else
  echo "[ltx23] WARNING: ltx23_triple_stage_workflow.json not found next to install script"
  echo "[ltx23] downloading official I2V workflow as fallback..."
  wget -q --show-progress -O "${WORKFLOW_DIR}/ltx23_i2v_official.json" \
    "https://raw.githubusercontent.com/Comfy-Org/workflow_templates/refs/heads/main/templates/video_ltx2_3_i2v.json" || true
fi
...
Workflow:
  Triple-stage I2V (community-proven, fp8 adapted)
  Loaded at: workflows/ltx23_i2v_triple_stage.json

Pipeline (3-stage):
  Stage 1: Image → LTX 2.3 fp8 + distilled LoRA → low-res gen (224x320 latent, 9 steps)
  Stage 2: Latent Upscale 2x → refine (4 steps)
  Stage 3: Latent Upscale 2x → refine (4 steps)
  Output: 30fps H.264 MP4

Settings:
  - Sampler: euler_ancestral_cfg_pp (stage 1), euler_cfg_pp (stages 2-3)
  - CFG: 1.0
  - LoRA distilled strength: 0.5
  - Frames: 97 (~3.2s at 30fps)
  - I2V denoise: 0.7
  - Input image: resized to 1536px longest side
