...
# Diffusion models (high + low noise, fp16 full precision for best quality)
# fp16 = ~28GB each, fits in 32GB VRAM with offloading
...
dl(REPO, "split_files/diffusion_models/wan2.2_i2v_low_noise_14B_fp16.safetensors",
...
echo "[wan22] Step 5/5: Configuring ComfyUI..."

# Set COMFYUI_ARGS environment variable for RTX 5090 compatibility
# The Vast supervisor script reads this variable to pass args to main.py
COMFY_LAUNCH="/opt/supervisor-scripts/comfyui.sh"
if [[ -f "${COMFY_LAUNCH}" ]] && ! grep -q "disable-async-offload" "${COMFY_LAUNCH}" 2>/dev/null; then
  # Patch the COMFYUI_ARGS default in the launch script
  sed -i 's|COMFYUI_ARGS=${COMFYUI_ARGS:---disable-auto-launch --port 18188 --enable-cors-header}|COMFYUI_ARGS=${COMFYUI_ARGS:---disable-auto-launch --port 18188 --enable-cors-header --disable-async-offload}|' \
    "${COMFY_LAUNCH}" 2>/dev/null || true
  echo "[wan22] Added --disable-async-offload to ComfyUI launch args"
fi
