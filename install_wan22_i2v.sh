...
#    - ComfyUI-Frame-Interpolation (FILM VFI, 16fps→32fps)
#    - RealESRGAN_x2plus upscaler (~67MB)
#
#  Total: ~40GB + ~67MB
# ============================================================================
...
# ── RealESRGAN_x2plus upscaler ──
mkdir -p "${COMFYUI_DIR}/models/upscale_models"
if [[ -s "${COMFYUI_DIR}/models/upscale_models/RealESRGAN_x2plus.pth" ]]; then
  echo "  [OK] RealESRGAN_x2plus.pth"
else
  echo "  [downloading] RealESRGAN_x2plus.pth (~67MB)..."
  wget -q -O "${COMFYUI_DIR}/models/upscale_models/RealESRGAN_x2plus.pth" \
...
for f in \
  ...
  "${COMFYUI_DIR}/models/upscale_models/RealESRGAN_x2plus.pth"; do
  if [[ -s "$f" ]]; then
    echo "  [OK] $(basename "$f")"
  else
...
================================================================
  ...
  - FILM VFI frame interpolation (16fps -> 32fps)
  - RealESRGAN_x2plus upscaler (848x480 -> 1920x1080)
