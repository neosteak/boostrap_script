...
# RIFE model (auto-download broken, manual required)
RIFE_DIR="${FI_DIR}/ckpts/rife"
mkdir -p "${RIFE_DIR}"
if [[ ! -s "${RIFE_DIR}/rife49.pth" ]]; then
  echo "[wan22] downloading: rife49.pth ..."
  wget -q -O "${RIFE_DIR}/rife49.pth" \
    "https://huggingface.co/Fannovel16/ComfyUI-Frame-Interpolation/resolve/main/ckpts/rife/rife49.pth" 2>/dev/null || true
fi
