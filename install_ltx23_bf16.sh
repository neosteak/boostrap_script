...
mkdir -p "$C/models/loras/ltxv/ltx2" 2>/dev/null
ln -sf "$C/models/loras/ltx-2.3-22b-distilled-lora-384.safetensors" \
       "$C/models/loras/ltxv/ltx2/ltx-2.3-22b-distilled-lora-384.safetensors" 2>/dev/null

# Symlink text encoders to clip folder (some nodes look there)
mkdir -p "$C/models/clip" 2>/dev/null
for f in "$C/models/text_encoders"/*.safetensors; do
    [ -f "$f" ] && ln -sf "$f" "$C/models/clip/$(basename $f)" 2>/dev/null
done
